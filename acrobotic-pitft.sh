...
function print_version() {
    echo "acrobotic PiTFT Helper v2.0.0"
    exit 1
}
...
function update_configtxt() {
    if grep -q "acrobotic-pitft-helper" "/boot/config.txt"; then
        echo "Already have an acrobotic-pitft-helper section in /boot/config.txt."
	echo "Removing old section..."
        cp /boot/config.txt /boot/configtxt.bak
	sed -i -e "/^# --- added by acrobotic-pitft-helper/,/^# --- end acrobotic-pitft-helper/d" /boot/config.txt
    fi
...
# --- added by acrobotic-pitft-helper $date ---
dtparam=spi=on
dtparam=i2c1=on
dtparam=i2c_arm=on
$overlay
# --- end acrobotic-pitft-helper $date ---
EOF
}
